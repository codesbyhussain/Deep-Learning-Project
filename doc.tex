...
The dataset is partitioned into train/validation/test sets using a 60/20/20 ratio with stratified random sampling (seed 0). Resulting split sizes: 36,120 train, 2,257 validation, 6,773 test. Split indices are generated once and serialized to disk (\texttt{splits.npz}), ensuring identical partitions across all experiments and machines.
